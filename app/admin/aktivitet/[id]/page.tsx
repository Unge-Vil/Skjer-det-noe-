...
export default async function ActivityFormPage({
  ...
  if (id !== "ny") {
    const { data } = await supabase
      .from("activities")
      .select(
        "id,title,title_en,description,description_en,category_id,category_ids,municipality_id,address,accessibility,area,ends_on,weekday,start_time,end_time,recurrence_note,age_min,age_max,price,image_url,status",
      )
      .eq("id", id)
      .maybeSingle();
    if (!data) notFound();
