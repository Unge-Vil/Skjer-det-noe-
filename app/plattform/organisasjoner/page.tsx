...
export default async function PlatformOrganisationsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <AdminShell title={t.platform.allOrgs} identity={<ContextSwitcher />} nav={platformNav(t)}>
    <main id="main" className="mx-auto w-full max-w-5xl" style={{ padding: 24 }}>
      ...
      <ul className="flex flex-col gap-2.5 sm:hidden" style={{ listStyle: "none", margin: 0, padding: 0 }}>{orgs.map((org) => <li key={org.id} style={{ border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", background: "var(--surface-card)", padding: 14, display: "flex", flexDirection: "column", gap: 10 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}><span style={{ fontWeight: 700, minWidth: 0 }}>{org.name}</span><span style={{ flex: "none", fontSize: "var(--fs-xs)", fontWeight: 700, color: org.status === "published" ? "var(--text-brand)" : "var(--text-muted)" }}>{org.status === "published" ? "Publisert" : "Venter"}</span></div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><span style={{ color: "var(--text-muted)", fontSize: "var(--fs-xs)" }}>{org.org_number ? `Org.nr. ${org.org_number}` : "–"}</span><OrgStatusButton id={org.id} status={org.status} /></div></li>)}</ul>
      <div className="hidden sm:block" style={tableWrap}><table style={tableStyle}><thead><tr><th style={th}>Organisasjon</th><th style={th}>Org.nr.</th><th style={th}>Status</th><th style={{ ...th, textAlign: "right" }}>Handling</th></tr></thead><tbody>{orgs.map((org) => <tr key={org.id} style={tr}><td style={{ ...td, fontWeight: 700 }}>{org.name}</td><td style={{ ...td, color: "var(--text-muted)" }}>{org.org_number ?? "–"}</td><td style={td}>{org.status === "published" ? "Publisert" : "Venter på godkjenning"}</td><td style={{ ...td, textAlign: "right" }}><OrgStatusButton id={org.id} status={org.status} /></td></tr>)}</tbody></table></div>
    </main>
  </AdminShell>;
}
