...
export async function syncCoOrganizers(
  ...
  if (toRemove.length) {
    await supabase.from(table).delete().eq(fkColumn, listingId).in("organization_id", toRemove);
  }
  if (toAdd.length) {
    await supabase.from(table).insert(toAdd.map((organization_id) => ({ [fkColumn]: listingId, organization_id })));
  }
}
