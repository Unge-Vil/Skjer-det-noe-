...
export default async function ApiIntegrationPage() {
  ...
  return <AdminShell title="API" identity={<ContextSwitcher />} nav={orgAdminNav(t)}>
    <main id="main" className="mx-auto w-full max-w-4xl" style={{ padding: 24 }}>
      ...
      <Link href="https://unge-vil.gitbook.io/skjer-det-noe/api-reference/" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 18, padding: "14px 16px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)", background: "var(--surface-card)", color: "var(--text-body)", textDecoration: "none" }}>
        <span><strong style={{ display: "block", marginBottom: 3 }}>{t.integrations.docsTitle}</strong><span style={{ color: "var(--text-muted)", fontSize: "var(--fs-sm)" }}>{t.integrations.docsIntro}</span></span>
